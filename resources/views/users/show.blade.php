    ...
    <div class="p-8 bg-gray-100 min-h-screen">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <h1 class="text-3xl font-bold text-gray-900">Editar Usuário: <span class="text-blue-600">{{ $user['name'] ?? '' }}</span></h1>
            ...
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8 px-8" aria-label="Tabs">
                    <button @click="tab = 'general'"
                        ...
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">Informações
                        Gerais</button>
                    <button @click="tab = 'groups'"
                        ...
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">Grupos e
                        Acesso</button>
                    <button @click="tab = 'permissions'"
                        ...
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">Permissões
                        Efetivas</button>
                </nav>
            </div>
            ...
            <form action="{{route('users.update', $user['uuid']) }} " method="POST" class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <input type="text" name="ra" id="ra" value="{{ $user['ra'] ?? '' }}"
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                <label for="recovery_code" class="block text-sm font-medium text-gray-700">Código de Recuperação</label>
                                <input type="text" name="recovery_code" id="recovery_code" value="{{ $user['recovery_code'] ?? '' }}"
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                        <div>
                        ...
                        <div>
                            ...
                            <label class="mt-2 inline-flex items-center cursor-pointer">
                                ...
                                <span
                                    class="ms-3 text-sm font-medium text-gray-900">{{ ($user['disabled'] ?? '0') == '0' ? 'Ativo' : 'Desativado' }}</span>
                            </label>
                        </div>
                    </div>
                </div>

                {{-- Aba 2: Grupos e Acesso --}}
                ...
                <div x-show="tab === 'permissions'" class="space-y-4">
                    <p class="text-sm text-gray-600">Selecione as permissões específicas que este usuário terá acesso no
                        sistema.</p>
                    <div class="border rounded-md p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2">
                        @foreach ($user['priv'] ?? [] as $privKey => $priv)
                            <label class="flex items-center font-normal">
                                <input type="checkbox" name="priv[]" value="{{ $privKey }}"
                                    {{ ($priv['selected'] ?? 0) == 1 ? 'checked' : '' }}
                                    class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
