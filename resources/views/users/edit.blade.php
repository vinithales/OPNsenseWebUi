...
<div class="p-8 bg-gray-100 min-h-screen">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Editar Usuário: <span class="text-indigo-600">{{ $user['name'] }}</span></h1>
    ...
    <div class="bg-white rounded-lg shadow-sm" x-data="{ tab: 'general' }">
        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8 px-8" aria-label="Tabs">
                <button @click="tab = 'general'" :class="{ 'border-indigo-500 text-indigo-600': tab === 'general', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': tab !== 'general' }"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">
                    Informações Gerais
                </button>
                <button @click="tab = 'groups'" :class="{ 'border-indigo-500 text-indigo-600': tab === 'groups', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': tab !== 'groups' }"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">
                    Grupos e Acesso
                </button>
                <button @click="tab = 'permissions'" :class="{ 'border-indigo-500 text-indigo-600': tab === 'permissions', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': tab !== 'permissions' }"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm focus:outline-none">
                    Permissões Efetivas
                </button>
            </nav>
        ...
        {{-- Formulário --}}
        <form action="{{ route('users.update', $user['uid']) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div x-show="tab === 'general'" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">Nome de Usuário (Login)</label>
                        <input type="text" name="name" id="name" value="{{ $user['name'] }}" readonly disabled
                    ...
                    <div>
                        <label for="expires" class="block text-sm font-medium text-gray-700">Data de Expiração (Opcional)</label>
                    ...
                    <div class="md:col-span-2">
                        <label for="authorizedkeys" class="block text-sm font-medium text-gray-700">Chaves SSH Autorizadas</label>
                        <textarea name="authorizedkeys" id="authorizedkeys" rows="4" class="mt-1 font-mono text-sm block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">{{ $user['authorizedkeys'] ?? '' }}</textarea>
                    </div>
                    <div>
                        ...
                        <label class="mt-2 inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="disabled" value="1" class="sr-only peer" {{ ($user['disabled'] == "0") ? '' : 'checked' }}>
                            ...
                            <span class="ms-3 text-sm font-medium text-gray-900">{{ ($user['disabled'] == "0") ? 'Ativo' : 'Desativado' }}</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div x-show="tab === 'groups'" class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="groups[]" value="{{ $group['value'] }}" {{ isset($group['selected']) && $group['selected'] ? 'checked' : '' }}
                ...
                <div class="space-y-6">
                    <div>
                        <label for="shell" class="block text-sm font-medium text-gray-700">Acesso ao Terminal (Shell)</label>
                        <select id="shell" name="shell" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 rounded-md">
                            @foreach(($user['shell'] ?? []) as $key => $shellInfo)
                            <option value="{{ $key }}" {{ isset($shellInfo['selected']) && $shellInfo['selected'] ? 'selected' : '' }}>
                                {{ $shellInfo['value'] }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label for="language" class="block text-sm font-medium text-gray-700">Idioma da Interface</label>
                        <select id="language" name="language" class="mt-1 block w-full pl-3 pr-10 py-2 border-gray-300 rounded-md">
                             @foreach(($user['language'] ?? []) as $key => $langInfo)
                            <option value="{{ $key }}" {{ isset($langInfo['selected']) && $langInfo['selected'] ? 'selected' : '' }}>
                                {{ $langInfo['value'] }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div x-show="tab === 'permissions'" class="space-y-4">
                <p class="text-sm text-gray-600">Selecione as permissões específicas que este usuário terá acesso no sistema.</p>
                ...
                <div x-data="{ open: false }" class="border rounded-md">
                    ...
                    <div x-show="open" x-cloak class="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 border-t">
                        ...
                        <label class="flex items-center font-normal">
                            <input type="checkbox" name="priv[]" value="{{ $privKey }}" {{ isset($priv['selected']) && $priv['selected'] ? 'checked' : '' }}
                                   class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
            ...
            <div class="mt-8 flex justify-end space-x-3 border-t pt-6">
                ...
                <button type="submit" class="flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-indigo-700">
