...
@section('main')
    ...
    <div class="p-8 bg-gray-100 min-h-screen">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <h1 class="text-3xl font-bold text-gray-900">Editar Grupo: <span
                            class="text-indigo-600">{{ $group['name'] ?? '' }}</span></h1>
                    <p class="text-gray-600">Altere a descrição, os membros e as permissões do grupo.</p>
                </div>
            </div>

            {{-- Botões Voltar + Excluir --}}
            <div class="flex items-center space-x-3">
                ...
                {{-- Excluir via JS --}}
                <button type="button"
                    class="flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-red-700"
        ...
        <form action="{{ route('groups.update', $group['uuid'] ?? '') }}" method="POST">
            ...
            <div class="bg-white rounded-lg shadow-sm p-8 space-y-8">
                ...
                <div>
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ $group['name'] ?? '' }}" readonly
                        ...
                        <select id="members-select" name="members[]" multiple placeholder="Selecione um ou mais usuários..."
                            ...
                            @if (isset($group['member']))
                                @foreach ($group['member'] as $userId => $memberData)
                                    <option value="{{ $userId }}"
                                        {{ ($memberData['selected'] ?? 0) == 1 ? 'selected' : '' }}>
                                        {{ $memberData['value'] }}
                                    </option>
                                @endforeach
                            @endif
                ...
                <div class="pt-6 mt-4 flex justify-end space-x-3 border-t border-gray-200">
                    ...
                    <button type="submit"
                        class="flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-indigo-700">
                        Salvar Alterações
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function deleteGroup(groupId) {
            if (!groupId) return alert('ID do grupo inválido');
